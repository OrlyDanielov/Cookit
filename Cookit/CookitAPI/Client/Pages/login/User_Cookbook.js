...
///מתכונים מועדפים
var USER_FAVORITE_RECIPES;
var USER_FAVORITE_RECIPES_DISPLAY = new Array();
var COUNT_FAVORITE_RECIPES = 1;
var COUNT_FAVORITE_NAME_RECIPES = 1;
//*******************************************************************************************
// PAGE LOAD
//*******************************************************************************************
...
function SuccessGetUserRecipes(data) {
    ...
    for (var i = 0; i < COUNT_RECIPES; i++) {
        USER_RECIPES_DISPLAY.push({
            ...
            img_name : USER_RECIPES[i].img_name,
            img_path : USER_RECIPES[i].img_path
        });
    }
    //הכנסת הלייקים
    ...
    //מביא את המתכונים המועדפים
    GetFavoriteUserRecipes();
    //הצגת כל המתכונים
    ShowRecipes();
}
...
//*******************************************************************************************
// GET FAVORITE USER RECIPE 
//*******************************************************************************************
function GetFavoriteUserRecipes() {
    GlobalAjax("/api/Recipe/GetFavoriteUserRecipes/" + LOGIN_USER.id, "GET", "", SuccessGetFavoriteUserRecipes, FailGetFavoriteUserRecipes);
}

function SuccessGetFavoriteUserRecipes(data) {
    console.log("משיכת נתוני מתכון בוצע בהצלחה!.");
    USER_FAVORITE_RECIPES = data;
    sessionStorage.setItem("USER_FAVORITE_RECIPES", JSON.stringify(data));
    COUNT_FAVORITE_RECIPES = data.length;
    for (var i = 0; i < COUNT_FAVORITE_RECIPES; i++) {
        USER_FAVORITE_RECIPES_DISPLAY.push({
            recp_id: USER_FAVORITE_RECIPES[i].recp_id,
            user_id: USER_FAVORITE_RECIPES[i].user_id,
            recp_name: USER_FAVORITE_RECIPES[i].recp_name,
            recp_dish_type: USER_FAVORITE_RECIPES[i].recp_dish_type,
            recp_dish_category: USER_FAVORITE_RECIPES[i].recp_dish_category,
            recp_food_type: USER_FAVORITE_RECIPES[i].recp_food_type,
            recp_kitchen_type: USER_FAVORITE_RECIPES[i].recp_kitchen_type,
            recp_level: USER_FAVORITE_RECIPES[i].recp_level,
            recp_total_time: USER_FAVORITE_RECIPES[i].recp_total_time,
            recp_work_time: USER_FAVORITE_RECIPES[i].recp_work_time,
            recp_steps: USER_FAVORITE_RECIPES[i].recp_steps,
            recp_holidays: null,
            recp_food_labels: null,
            recp_owner_name: null,
            login_user_favorite: false,
            login_user_like: false,
            recp_count_like: null,
            img_name : USER_FAVORITE_RECIPES[i].img_name,
            img_path : USER_FAVORITE_RECIPES[i].img_path
        });
    }
    //הכנסת הלייקים
    for (var h = 0; h < USER_FAVORITE_RECIPES_DISPLAY.length; h++) {
        var flag = false;
        for (var i = 0; i < USER_LIKE.length && (!flag); i++) {
            if (USER_LIKE[i].id_recipe == USER_FAVORITE_RECIPES_DISPLAY[h].recp_id) {
                flag = true;
                USER_FAVORITE_RECIPES_DISPLAY[h].login_user_like = true;
            }
        }
    }
    //הכנסת המועדפים
    for (var a = 0; a < USER_FAVORITE_RECIPES_DISPLAY.length; a++) {
        var flag_a = false;
        for (var b = 0; b < USER_FAVORITE.length && (!flag_a); b++) {
            if (USER_FAVORITE[b].id_recipe == USER_FAVORITE_RECIPES_DISPLAY[a].recp_id) {
                USER_FAVORITE_RECIPES_DISPLAY[a].login_user_favorite = true;
                flag_a = true;
            }
        }
    }
...
function FailGetFavoriteUserRecipes(data) {
    console.log("error! can't get recipe information.");
    console.log(data);
    alert("שגיאה במשיכת נתוני מתכון!, אנא נסה שנית מאוחד יותר.");
...
function ShowFavoriteRecipes() {
    document.getElementById("user_favorite_recipes").innerHTML = "";
    COUNT_FAVORITE_NAME_RECIPES = 1;
    if (USER_FAVORITE_RECIPES_DISPLAY.length == 0) //אם אין מתכונים
        document.getElementById("user_favorite_recipes").innerHTML = "אין מתכונים";
    else {
        for (var i = 0; i < USER_FAVORITE_RECIPES_DISPLAY.length; i++) {
            GetProfileName_FavoriteRecipe(USER_FAVORITE_RECIPES_DISPLAY[i].user_id, USER_FAVORITE_RECIPES_DISPLAY[i]);
        }
    }
}
...
// IS USER HAS PROFILE
//*******************************************************************************************
function GetProfileName_FavoriteRecipe(_user_id, _recipe) {
    GlobalAjax("/api/Profile/GetProfileByUserId/" + _user_id, "GET", "",
        function (data) {
            console.log("שם פרופיל " + data.name);
            _recipe.recp_owner_name = data.name;
            CountLikeOfRecipe_FavoriteRecipe(_recipe, data.name);
        },
        function () {
            console.log("לא מצא את הפרופיל, מחפפש אחר שם משתמש");
            GetFullUserNameById_FavoriteRecipe(_user_id, _recipe);
        });
}
//*******************************************************************************************
// Get Full User NameBy Id
//*******************************************************************************************
function GetFullUserNameById_FavoriteRecipe(_user_id, _recipe) {
    GlobalAjax("/api/User/GetUserFullNameByID/" + _user_id, "GET", "",
        function (data) {
            console.log("שם משתמש " + data);
            _recipe.recp_owner_name = data;
            CountLikeOfRecipe_FavoriteRecipe(_recipe, data);
        }, function () {
            console.log("בעיה במשיגת שם יוצר מתכון!");
            CountLikeOfRecipe_FavoriteRecipe(_recipe, null);
        });
}
//*******************************************************************************************
// Get number of like of recipe
//*******************************************************************************************
function CountLikeOfRecipe_FavoriteRecipe(_recipe, _name) {
    GlobalAjax("/api/Like/GetCountLikeOfRecipe/" + _recipe.recp_id, "GET", "",
        function (data) {
            console.log("מספר לייקים " + data);
            _recipe.recp_count_like = data;
            AddFavoriteRecipe(_recipe);
        },
        function () {
            console.log("שגיאה! לא מצליח להביא את מספר הלייקים של המתכון");
            AddFavoriteRecipe(_recipe);
...
// Add Recipe
//*******************************************************************************************
function AddFavoriteRecipe(_recipe) {
    //div
    var div = document.createElement('div');
    div.className = "col-md-4 ";
    //div
    ...
    //הוספת השלב לתצוגה
    div.appendChild(recipe_div);
    console.log(COUNT_FAVORITE_NAME_RECIPES % 3);
    if (COUNT_FAVORITE_NAME_RECIPES % 3 == 1) {
        //create new row
        var row = document.createElement("div");
        row.className = "row";
        console.log(Math.ceil(COUNT_FAVORITE_NAME_RECIPES / 3));
        row.id = "favorite_row_" + Math.ceil(COUNT_FAVORITE_NAME_RECIPES / 3); // עיגול למעלה

        row.appendChild(div);
        document.getElementById("user_favorite_recipes").appendChild(row);
    }
    else {
        console.log(Math.ceil(COUNT_FAVORITE_NAME_RECIPES / 3));
        document.getElementById("favorite_row_" + Math.ceil(COUNT_FAVORITE_NAME_RECIPES / 3)).appendChild(div);
    }
    console.log("recipe " + COUNT_FAVORITE_NAME_RECIPES);
    COUNT_FAVORITE_NAME_RECIPES += 1;
}
