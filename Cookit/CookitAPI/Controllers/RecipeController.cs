...
namespace Cookit.Controllers
{
    ...
    public class RecipeController : ApiController
    {
        ...
        public HttpResponseMessage GetAllRecipes()
        {
            try
            {
                List<TBL_Recipe> list_recipes = CookitQueries.GetAllRecipes();
        ...
        public HttpResponseMessage GetRecommendedRecipes()
        {
            try
            {
                ...
                List<TBL_Recipe> recipe_list = CookitQueries.GetAllRecipes();
                if (recipe_list == null )
                ...
                else
                {
                    ...
                    {
                        like_count = CookitQueries.GetCountLikeOfRecipe(item.Id_Recipe);//מביא את מספר הלייקים של מתכון
        ...
        public HttpResponseMessage GetRecipesOfFollowProfiles(int user_id)
        {
            try
            {
                ...
                List<TBL_Followers> user_follow_list = CookitQueries.GetProfileFollowByUser(user_id);
                ...
                if (user_follow_list.Count >0)
                {
                    ...
                    foreach (TBL_Followers follow in user_follow_list)
                    {
                        ...
                        profile = CookitQueries.GetProfileByProfileId(profile_id); // מביא את הפרופיל התואם
                        if (profile != null)
                        {
                            //מביא את המתכונים של הפרופיל הנוכחי
                            List<TBL_Recipe> profile_recipes = CookitQueries.GetRecipesByUserId(profile.Id_User);//מביא את המתכונים של הפרופיל
        ...
        public HttpResponseMessage Get_FU_Recipes()
        {
            try
            {
                ...
                List<TBL_User> FU_list = CookitQueries.GetFU();
        ...
        public HttpResponseMessage GetRecpByUserIdAndRecpName(int user_id, string recp_name)
        {
            try
            {
                TBL_Recipe recipe = CookitQueries.GetRecpByUserIdAndRecpName(user_id, recp_name);
               if(recipe == null)
                    return Request.CreateResponse(HttpStatusCode.NotFound, "this recipe does not exist.");
               else
                {
        ...
        public HttpResponseMessage GetRecipesByUserId(int user_id)
        {
            try
            {
                List<TBL_Recipe> recipes = CookitQueries.GetRecipesByUserId(user_id);
                if (recipes == null)
                    return Request.CreateResponse(HttpStatusCode.NotFound, "this recipe does not exist.");
                else
                {
        ...
        public HttpResponseMessage GetRecpByUserIdRecipe(int recipe_id)
        {
            try
            {
                TBL_Recipe recipe = CookitQueries.GetRecpByUserIdRecipe(recipe_id);
                if (recipe == null)
                    return Request.CreateResponse(HttpStatusCode.NotFound, "this recipe does not exist.");
                else
                {
                    ...
                    result.recp_kitchen_type = recipe.Id_Recipe_KitchenType;
                    result.recp_level = recipe.Id_Recipe_Level;
                    result.recp_total_time = recipe.RecipeTotalTime; 
                    result.recp_work_time = recipe.RecipeWorkTime;
                    result.recp_steps = recipe.PreparationSteps;
                    result.img_name = recipe.Image_Name;
                    result.img_path = recipe.Image_Path;
        ...
        public HttpResponseMessage GetFavoriteUserRecipes(int user_id)
        {
            try
            {
                ...
                List<TBL_FavoriteRecp> favorite_recp_list = CookitQueries.GetFavoriteByUserId(user_id);
                //המתכונים שמשתמש לא העלאה
                List<TBL_Recipe> recipe_list = CookitQueries.GetAllRecipesExpectOfUserId(user_id);
        ...
        public HttpResponseMessage AddNewRecipe([FromBody]RecipeDTO newRecipe)
        {
            try
            {
                ...
                var new_id = CookitQueries.AddNewRecipe(r);
                if (new_id != -1)
        ...
        public void AddRecipeImage()
        {
            ...
            bool is_updated = CookitQueries.UpdateRecipeImage(recipe_id, img_name, pull_path);
        ...
        public HttpResponseMessage UpdateRecipe([FromBody]RecipeDTO recipe)
        {
            try
            {
                ...
                var is_saved = CookitQueries.UpdateRecipe(r);
                if (is_saved == true)
